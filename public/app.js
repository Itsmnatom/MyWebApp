...
async function handleLocation() {
    ...
    // Reset views
    ['home-view', 'detail-view', 'reader-view', 'history-view', 'bookmarks-view', 'alt-view'].forEach(id => {
    ...
    if (path === '/read' && targetUrl) {
        ...
        bView.classList.add('animate-fade-in-up');
        renderBookmarksPage();
    } else if (path === '/alt') {
        const aView = document.getElementById('alt-view');
        if (aView) {
            aView.classList.remove('hidden');
            aView.classList.add('animate-fade-in-up');
        }
        renderAltPage();
            ...
            <div onclick="navigate('/read?url=${encodeURIComponent(h.chapterUrl)}&mangaUrl=${encodeURIComponent(h.mangaUrl)}&title=${encodeURIComponent(`${clean(h.title)} - ${clean(h.chapterName)}`)}')"
                ...
                <div class="flex flex-col justify-center min-w-0 flex-1">
                    ...
                    <div class="flex items-center gap-2 text-[10px] text-gray-500 font-bold uppercase">
                        <i class="far fa-clock"></i> <span>Recorded: ${new Date(h.time).toLocaleDateString()}</span>
                    </div>
                </div>
            </div>
        `).join('');
...
async function renderDetail(url) {
    ...
    try {
        ...
        const fetchUrl = isAlt 
            ...
            : `${API}/manga/details?url=${encodeURIComponent(url)}`;
        ...
        document.getElementById('d-info').innerHTML = Object.entries(d.info || {}).map(([k, v]) => `
            ...
            <div class="glass px-4 py-2 rounded-xl flex items-center gap-3 hover:bg-white/5 transition-colors">
                <span class="text-amber-500 font-black uppercase tracking-widest text-[9px] opacity-80">Status</span>
                <span class="text-gray-200 font-medium text-xs">${clean(d.status)}</span>
            </div>` : '') + (d.type ? `
            <div class="glass px-4 py-2 rounded-xl flex items-center gap-3 hover:bg-white/5 transition-colors">
                <span class="text-blue-400 font-black uppercase tracking-widest text-[9px] opacity-80">Type</span>
...
async function renderReader(url, title) {
    ...
    try {
        ...
        const isAlt = url.includes('1668manga.com') || new URLSearchParams(window.location.search).get('alt');
